...
在浏览器中，我们通过事件监听来实现 jsxx 和 HTML 之间的交互。一个页面往往会被绑定许许多多的事件，而页面接收事件的顺序，就是**事件流**。
...
![Ciqc1F_KCc2AH3SuAADAfZ2rEXk066](assets/Ciqc1F_KCc2AH3SuAADAfZ2rEXk066.png)
...
![Ciqc1F_KGs2AQ6VHAABqyZCa5L0820](assets/Ciqc1F_KGs2AQ6VHAABqyZCa5L0820.png)
...
![CgqCHl_KCfyAEJw8AAFeUK36DtI133](assets/CgqCHl_KCfyAEJw8AAFeUK36DtI133.png)

e.nativeEvent 将会输出 MouseEvent 这个原生事件，如下图所示：

![CgqCHl_KCgOAZixsAACyXDjo4cs933](assets/CgqCHl_KCgOAZixsAACyXDjo4cs933.png)
...
![Ciqc1F_KCi-ANLdOAACnuwItpnA575](assets/Ciqc1F_KCi-ANLdOAACnuwItpnA575.png)

这些函数之间是如何各司其职、打好“配合”的呢？请看下面这张工作流大图：

![CgqCHl_KCjaALFKsAAHNjlT3rrw342](assets/CgqCHl_KCjaALFKsAAHNjlT3rrw342.png)
...
![Ciqc1F_KCj6ABv8CAAMpWS1ws8Q056](assets/Ciqc1F_KCj6ABv8CAAMpWS1ws8Q056.png)
...
![CgqCHl_KCkWACHkwAACydURG2fM056](assets/CgqCHl_KCkWACHkwAACydURG2fM056.png)
...
![Ciqc1F_KCk-Afe7QAADMlUnc-KU093](assets/Ciqc1F_KCk-Afe7QAADMlUnc-KU093.png)

在这段逻辑中，element 就是 document 这个 DOM 元素，如下图所示，它在 legacyListenToEvent 阶段被获取后，又被层层的逻辑传递到了这个位置。

![Ciqc1F_KClaATXVLAAAhVJ6fu48519](assets/Ciqc1F_KClaATXVLAAAhVJ6fu48519.png)
...
![Ciqc1F_KCl-AQUfuAAA3s44GMaI290](assets/Ciqc1F_KCl-AQUfuAAA3s44GMaI290.png)

addEventListener 就更不用多说了，它是[原生 DOM 里专门用来注册事件监听器的接口](https://developer.mozilla.org/zh-CN/docs/Web/API/EventTarget/addEventListener)。我们真正需要关注的是图中这个函数的前两个入参，首先看 eventType，它表示事件的类型，这里我监听的是一个点击事件，因此 eventType 就是 click（见下图的运行时输出结果）。

![Ciqc1F_KCmeARunyAACxP1sCnKY948](assets/Ciqc1F_KCmeARunyAACxP1sCnKY948.png)
...
![Ciqc1F_KCneAfMZbAAE9PxK7X3w813](assets/Ciqc1F_KCneAfMZbAAE9PxK7X3w813.png)
...
![CgqCHl_KCoaAHCSUAAAdOFZY7GY194](assets/CgqCHl_KCoaAHCSUAAAdOFZY7GY194.png)

界面中渲染出来的是一行数字文本和一个按钮，每点击一下按钮，数字文本会 +1。在 jsxX 结构中，监听点击事件的除了 button 按钮外，还有 id 为 container 的 div 元素，这个 div 元素同时监听了点击事件的冒泡和捕获。

App 组件对应的 Fiber 树结构如下图所示：

![CgqCHl_KCuSAdlxpAADdtt6LkCw620](assets/CgqCHl_KCuSAdlxpAADdtt6LkCw620.png)
...
![Ciqc1F_KCz6AFPppAADdoA4lHx0444](assets/Ciqc1F_KCz6AFPppAADdoA4lHx0444.png)

因此最后收集上来的 path 数组内容就是 div#container、div.App 及 button 节点自身（button 节点别忘了，它是 while 循环的起点，一开始就会被推进 path 数组），如下图所示：

![Ciqc1F_KC0SAHu4MAAC4V-Al5nU582](assets/Ciqc1F_KC0SAHu4MAAC4V-Al5nU582.png)
...
![CgqCHl_KC1GAecTRAAFEgV7Sms0914](assets/CgqCHl_KC1GAecTRAAFEgV7Sms0914.png)
