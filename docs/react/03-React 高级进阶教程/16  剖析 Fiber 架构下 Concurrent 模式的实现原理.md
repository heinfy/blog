...
![CgqCHl_GIlaAHn_FAAAchawzt4s009](assets/CgqCHl_GIlaAHn_FAAAchawzt4s009.png)
...
![Ciqc1F_GK5KAPsNRAADXbk-KIQg893](assets/Ciqc1F_GK5KAPsNRAADXbk-KIQg893.png)

待 commit 阶段完成后，右侧的 workInProgress 树对应的 DOM 树就被真正渲染到了页面上，此时 current 指针会指向 workInProgress 树：

![CgqCHl_GImqABNjLAACxddF2jhY942](assets/CgqCHl_GImqABNjLAACxddF2jhY942.png)
...
![CgqCHl_GInGAAhaiAADrvJFiOdA159](assets/CgqCHl_GInGAAhaiAADrvJFiOdA159.png)

这段复用的逻辑在 beginWork 调用链路中的 createWorkInProgress 方法里。这里我为你截取了 createWorkInProgress 方法里面一段非常关键的逻辑，请看下图：

![CgqCHl_GIneAPBZcAAKQF1HrIv0329](assets/CgqCHl_GIneAPBZcAAKQF1HrIv0329.png)
...
![Ciqc1F_GIn2AK_7QAADrvJFiOdA218](assets/Ciqc1F_GIn2AK_7QAADrvJFiOdA218.png)
...
![Ciqc1F_GIoSAIIE5AAJG_0ANgrg415](assets/Ciqc1F_GIoSAIIE5AAJG_0ANgrg415.png)
...
![CgqCHl_GIouAVjKDAAEwJVqKwHY353](assets/CgqCHl_GIouAVjKDAAEwJVqKwHY353.png)
...
![CgqCHl_GIpSAfb9qAACwnR8L9Po005](assets/CgqCHl_GIpSAfb9qAACwnR8L9Po005.png)
...
![CgqCHl_GIpqAFVQWAAE4Rdg6o0g228](assets/CgqCHl_GIpqAFVQWAAE4Rdg6o0g228.png)

请你关注图中两处标红的函数调用，你会看到 dispatchAction 方法在 performSyncWorkOnRoot 的左边。也就是说整体的更新链路应该是这样的：

![Ciqc1F_GIqKAP_3fAABp3EtlwDk160](assets/Ciqc1F_GIqKAP_3fAABp3EtlwDk160.png)

请你关注图中两处标红的函数调用，你会看到 dispatchAction 方法在 performSyncWorkOnRoot 的左边。也就是说整体的更新链路应该是这样的：

![CgqCHl_GIqiAOf4aAAQVWAWjjt0722](assets/CgqCHl_GIqiAOf4aAAQVWAWjjt0722.png)
...
![CgqCHl_GIrCAJiUaAACZL_H8ts8659](assets/CgqCHl_GIrCAJiUaAACZL_H8ts8659.png)
...
![CgqCHl_GIrmAO5UJAABDaDLvIyM528](assets/CgqCHl_GIrmAO5UJAABDaDLvIyM528.png)
...
![CgqCHl_GIsGADUaBAAAYE3Ps56g927](assets/CgqCHl_GIsGADUaBAAAYE3Ps56g927.png)
...
![Ciqc1F_GIseASLuzAAVy6vkOrOA579](assets/Ciqc1F_GIseASLuzAAVy6vkOrOA579.png)
...
![Ciqc1F_GItCAerpyAAQXnhAd3k4018](assets/Ciqc1F_GItCAerpyAAQXnhAd3k4018.png)
...
在 else 中，需要引起你注意的是 ensureRootIsScheduled 这个方法，该方法很关键，它将决定如何开启当前更新所对应的 render 阶段。在 ensureRootIsScheduled 中，有[这样一段核心逻](https://github.com/facebook/react/blob/b6df4417c79c11cfb44f965fab55b573882b1d54/packages/react-reconciler/src/ReactFiberWorkLoop.new.jsx#L602)辑（解析在注释里）：
...
![Ciqc1F_GIt-AVQdjAAEHWIrq_Po945](assets/Ciqc1F_GIt-AVQdjAAEHWIrq_Po945.png)
...
function App() {
  ...
  const renderContent = arr.map((i, index) => <p style={{ width: 128, textAlign: 'center' }}>{`测试文本第${index}行`}</p>);
  return (
    <div className='App'>
...
![Ciqc1F_GIuyABtYCAABNHJ0zT3I546](assets/Ciqc1F_GIuyABtYCAABNHJ0zT3I546.png)

当我使用 ReactDOM.render 来渲染这个长列表时，它的调用栈如下图所示：

![Ciqc1F_GIvOALbOqAAOn8pi1lpw155](assets/Ciqc1F_GIvOALbOqAAOn8pi1lpw155.png)

在这张图中，你就不必再重复去关注 beginWork、completeWork 之流了，请把目光放在调用栈的上层，也就是图中标红的地方——一个不间断的灰色“Task”长条，对浏览器来说就意味着是一个**不可中断**的任务。

在我的浏览器上，这个 Task 的执行时长在 130ms 以上（将鼠标悬浮在 Task 长条上就可以查看执行时长）。而**浏览器的刷新频率为 60Hz，也就是说每 16.6ms 就会刷新一次**。在这 16.6ms 里，除了 jsx 线程外，渲染线程也是有工作要处理的，**但超长的 Task 显然会挤占渲染线程的工作时间**，引起“掉帧”，进而带来卡顿的风险，这也正是第 12 讲中所提到的“**jsx 对主线程的超时占用**”问题。

若将 ReactDOM.render 调用改为 createRoot 调用（即开启 Concurrent 模式），调用栈就会变成下面这样：

![Ciqc1F_GIvuATEYcAALRQfAuBFI173](assets/Ciqc1F_GIvuATEYcAALRQfAuBFI173.png)
...
![CgqCHl_GIwKAHTjDAACgXrFd5c0290](assets/CgqCHl_GIwKAHTjDAACgXrFd5c0290.png)
...
![CgqCHl_GIwiAAsOJAADS76o4FHc058](assets/CgqCHl_GIwiAAsOJAADS76o4FHc058.png)
...
![Ciqc1F_GIxeAJas8AAEiY1Hfbsw871](assets/Ciqc1F_GIxeAJas8AAEiY1Hfbsw871.png)
...
![Ciqc1F_GIx-AL94QAAHs2HJQXfA804](assets/Ciqc1F_GIx-AL94QAAHs2HJQXfA804.png)
...
![CgqCHl_GIzeAHilIAAFT8rmskL8314](assets/CgqCHl_GIzeAHilIAAFT8rmskL8314.png)
